...
{ 
	...
	*/
	...
	while(1) {
		...
		autoMovimento[indiceSenso]++;

		while (ponteOccupato || myBiglietto > biglietto[indiceSenso]
			|| autoMovimento[indiceSenso] < autoMovimento[!indiceSenso]
			|| (autoMovimento[indiceSenso] == autoMovimento[!indiceSenso] && indiceSenso == INDICESENSOANTIORARIO)) {
