...
/* SUGGERIMENTO: come sapere quanti aerei voglio atterrare? */

int AtterraggioAerei = 0;
...
{ 
	...
	while(1) {
		...
		while (PistaLibera == 0 || AtterraggioAerei > 0) {
		 ...
		 * e che quindi c'e' uno in piu' a cui 
		 * chi vuole decollare deve dare la precedenza*/
		...
		/* VIC: dovevi anticipare la lock QUI, altrimenti accedi
			alla variabile condivisa Atterraggio Aerei NON in mutua esclusione
		*/
		DBGpthread_mutex_lock(&mutex, Plabel);
		 ...
		 * e che c'e' uno in meno ad avere la precedenza 
		 * su chi vorrebbe decollare 
		 */
		...
		/* QUINTA PARTE DA COMPLETARE DA QUI */

		DBGpthread_mutex_unlock(&mutex, Plabel);

		/* FINE QUINTA PARTE DA COMPLETARE FINO A QUI */


		/* ora aereo va a rifornirsi */
