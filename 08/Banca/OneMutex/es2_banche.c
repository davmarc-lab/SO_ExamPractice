...
{ 
	...
	while(1) 
	{
		...
		if (rc != 0) PrintERROR_andExit(rc, "Depositi lock error");
		...
		if (rc != 0) PrintERROR_andExit(rc, "Depositi unlock error");
...
{ 
	...
	while(1) 
	{
		...
		rc = pthread_mutex_lock(&mutex);
		if (rc != 0) PrintERROR_andExit(rc, "Prelievi lock error");
		...
		rc = pthread_mutex_unlock(&mutex);
		if (rc != 0) PrintERROR_andExit(rc, "Prelievi unlock error");
...
{ 
	...
	while(1) 
	{
		...
		rc = pthread_mutex_lock(&mutex);
		if (rc != 0) PrintERROR_andExit(rc, "BancadItalia lock error");
		...
		*/
		...
		rc = pthread_mutex_unlock(&mutex);
		if (rc != 0) PrintERROR_andExit(rc, "bancadItalia unlock error");
